...
%% dabs.generic.BeamModulatorFastAnalog (920nm Power P1)
...
%% dabs.generic.BeamModulatorFastAnalog (1050nm Power P1)
AOControl = '/vDAQ0/AO3';     % control terminal  e.g. '/vDAQ0/AO0'
AIFeedback = '/vDAQ0/AI3';     % feedback terminal e.g. '/vDAQ0/AI0'
...
%% dabs.generic.FastZPureAnalog (ETL)
...
travelRangeUm = [0 100];     % travel range in micron

voltsPerUm = 0.1;     % volts per micron
...
amplitudeToFrequencyMap = [26 7921.66];     % translates an amplitude (degrees) to a resonant frequency (Hz)
amplitudeLUT = zeros(0,2);     % translates a nominal amplitude (degrees) to an output amplitude (degrees)

minimizeOnStart = false;

%% dabs.generic.DigitalShutter (920 P1)
...
%% dabs.generic.DigitalShutter (1050 P1)
DOControl = '/vDAQ0/D0.3';     % control terminal  e.g. '/vDAQ0/DIO0'
invertOutput = false;     % invert output drive signal to shutter
openTime_s = 0.5;     % settling time for shutter in seconds
shutterTarget = 'Excitation';     % one of {', 'Excitation', 'Detection'}

%% scanimage.components.scan2d.RggScan (RGG_P1)
...
beams = {'920nm Power P1' '1050nm Power P1'};     % beam device names
fastZs = {'ETL'};     % fastZ device names
shutters = {'920 P1' '1050 P1'};     % shutter device names
...
% Signal Conditioning
disableMaskDivide = [0 0 0 0];     % disable averaging of samples into pixels; instead accumulate samples
...
%% dabs.zaber.ZaberMultiDevice (ZaberMotor)
...
%% dabs.generic.recolorGuis (Recolor Path 1)
scanners = {'RGG_P1'};     % Order of imaging scanners associated with the order of colors. Ordinarily, this should not be modified by the user.
