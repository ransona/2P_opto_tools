...
parser.addParameter('AutoStageStepUm', [250 250], @(x) isnumeric(x) && numel(x) == 2 && all(isfinite(x)));
...
ui.maxIterationsEdit = uicontrol(mainPanel, 'Style', 'edit', 'String', '5', ...
    ...
    uicontrol(mainPanel, 'Style', 'text', 'String', 'Min Improve (ref)', ...
    'HorizontalAlignment', 'left', 'Units', 'normalized', 'Position', [0.82 0.94 0.10 0.03]);
ui.minImproveEdit = uicontrol(mainPanel, 'Style', 'edit', 'String', '0.05', ...
    'Units', 'normalized', 'Position', [0.92 0.94 0.05 0.04]);

uicontrol(mainPanel, 'Style', 'text', 'String', 'Max Match (ref)', ...
...
ui.iterationTable = uitable('Parent', mainPanel, ...
    ...
    'ColumnName', {'Iter', 'Matches', 'Measured RMS (ref)', 'Fit Residual RMS (ref)', 'Confirmed Improve (ref)', 'Accepted'}, ...
...
onLoadPattern();
    ...
    function runCalibrationWithMaxIterations(maxIterations)
        ...
        try
            ...
            options.MinImprovementUm = parseNumericField(ui.minImproveEdit, 0.05);
    ...
    function ensureDetectCallbackConfigured(localCfg)
        if isempty(localCfg.DetectHolesFcn)
            error('No DetectHolesFcn configured. Provide a callback that returns detected hole centers in native reference-space coordinates.');
        end
    end
