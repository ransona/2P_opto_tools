...
def build_test_stim_waveform_command(path_config: PathConfig) -> str:
    ...
    return "\n".join(
        [
            ...
            f"do_task = opto.scanimage.testVdaqDoTriggeredByDi('outputLine', {matlab_string(path_config.trial_waveform_output_port.split('/')[-1])}, 'startTrigger', '', 'sampleRate_Hz', {path_config.trial_waveform_sample_rate_hz!r}, 'pulseTimes_s', [0.1 1.1 2.1 3.1 4.1], 'pulseWidth_s', {path_config.trial_waveform_pulse_width_ms / 1000.0!r});",
            "t0 = tic;",
            "while most.idioms.isValidObj(do_task) && ~do_task.isTaskDone() && toc(t0) < 7.0; pause(0.01); end",
...
def build_test_stim_waveform_external_start_command(path_config: PathConfig) -> str:
    ...
    return "\n".join(
        [
            ...
            "disp([0.1 1.1 2.1 3.1 4.1]);",
            f"do_task = opto.scanimage.testVdaqDoTriggeredByDi('outputLine', {matlab_string(path_config.trial_waveform_output_port.split('/')[-1])}, 'startTrigger', {matlab_string(path_config.trial_waveform_start_trigger_port.split('/')[-1])}, 'sampleRate_Hz', {path_config.trial_waveform_sample_rate_hz!r}, 'pulseTimes_s', [0.1 1.1 2.1 3.1 4.1], 'pulseWidth_s', {path_config.trial_waveform_pulse_width_ms / 1000.0!r});",
