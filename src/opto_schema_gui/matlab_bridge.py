...
def build_online_analysis_delta_command(
    ...
    return "\n".join(
        [
            ...
            "        valueHistory = hInt.integrationValueHistoryPostProcessed;",
            "        timestampHistory = hInt.integrationTimestampHistory;",
            "        frameNumberHistory = hInt.integrationFrameNumberHistory;",
            ...
            "            end",
            "            values{idx} = reshape(double(valueHistory(sampleIdx, idx)), 1, []);",
            "            timestamps{idx} = reshape(double(timestampHistory(sampleIdx, idx)), 1, []);",
            "            frameNumbers{idx} = reshape(double(frameNumberHistory(sampleIdx, idx)), 1, []);",
            "        end",
