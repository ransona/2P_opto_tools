...
def build_begin_slm_psf_diagnostic_command(
    ...
    *,
    ...
    lines = [
        ...
        "try; backup.zs = double(hSI.hStackManager.zs); catch; end",
        "backup.mroiEnable = [];",
...
def build_run_slm_psf_volume_command(
    ...
    *,
    ...
    lines = [
        ...
        f"hSI.hStackManager.numSlices = {int(num_slices)};",
        f"hSI.hStackManager.framesPerSlice = {int(frames_per_slice)};",
        f"halfCount = floor({int(num_slices)} / 2);",
        f"zs = ({float(z_um)!r}) + ((0:{int(num_slices) - 1}) - halfCount) .* {float(z_step_um)!r};",
        "hSI.hStackManager.zs = zs;",
        ...
        "if isprop(hPs, 'monitoring'); hPs.monitoring = true; end",
        "if isprop(hPs, 'logging'); hPs.logging = true; end",
...
def build_restore_slm_psf_diagnostic_command(path_config: PathConfig) -> str:
    ...
    lines = [
        ...
        "try; if ~isempty(backup.zs); hSI.hStackManager.zs = backup.zs; end; catch; end",
