...
def build_trigger_photostim_command(
    ...
    lines = [
        ...
        "idlePosition = currentPosition + numel(trialTailSingle);",
        "assert(idlePosition <= numel(currentSequence), 'Precomposed active photostim sequence does not contain enough remaining trials. Run prep_patterns again.');",
        "existingTail = currentSequence(insertPosition:idlePosition);",
        "assert(isequal(double(existingTail(:).'), double(trialTailSingle(:).')), 'Prepared active photostim sequence does not match the requested trial tail. Run prep_patterns again.');",
