...
def build_test_stim_waveform_command(path_config: PathConfig) -> str:
    ...
    return "\n".join(
        [
            ...
            "disp('Configured waveform function:');",
            ...
            *resource_setup,
            ...
            "wg.dutyCycle = 50;",
            "wg.startTriggerPort = '';",
            "disp('Waveform task control line:');",
            "disp(string(wg.hControl.name));",
            "disp('Waveform task start trigger line:');",
            "if most.idioms.isValidObj(wg.startTriggerPort); disp(string(wg.startTriggerPort.name)); else; disp('<none>'); end",
            "disp('Waveform task sample rate:');",
            "disp(wg.sampleRate_Hz);",
            "widthsSec = [0.010 0.050 0.100 0.250 0.500];",
            "disp('Single-pulse width sweep sec:');",
            "disp(widthsSec);",
            "for widthIdx = 1:numel(widthsSec)",
            "    trialTriggerTimesSec = 0.1;",
            "    trialPulseWidthSec = widthsSec(widthIdx);",
            "    trialTotalDurationSec = max(1.0, trialPulseWidthSec + 0.2);",
            "    assignin('base', 'photostimTrialTriggerTimesSec', trialTriggerTimesSec);",
            "    assignin('base', 'photostimTrialPulseWidthSec', trialPulseWidthSec);",
            "    assignin('base', 'photostimTrialTotalDurationSec', trialTotalDurationSec);",
            "    if most.idioms.isValidObj(wg.hTask) && wg.hTask.active; wg.hTask.stop(); end",
            ...
            "    wg.refreshWvfmParams();",
            "    waveformSequencePositionBefore = [];",
            "    waveformCompletedSequencesBefore = [];",
            "    if hPs.active;",
            "        waveformSequencePositionBefore = hPs.sequencePosition;",
            "        waveformCompletedSequencesBefore = hPs.completedSequences;",
            "    end",
            "    wv = wg.computeWaveform();",
            "    disp('Single pulse width sec:');",
            "    disp(trialPulseWidthSec);",
            "    disp('Waveform buffer length samples:');",
            "    disp(numel(wv));",
            "    disp('Waveform buffer high samples:');",
            "    disp(sum(wv > 0));",
            "    wg.startTask();",
            "    disp('Waveform single-pulse test task started');",
            "    t0 = tic;",
            "    while most.idioms.isValidObj(wg.hTask) && ~wg.hTask.isTaskDone() && toc(t0) < (trialTotalDurationSec + 1.0); pause(0.01); end",
            "    disp('Waveform task active after single pulse:');",
            "    if most.idioms.isValidObj(wg.hTask); disp(double(wg.hTask.active)); else; disp(0); end",
            "    disp('Waveform task done after single pulse:');",
            "    if most.idioms.isValidObj(wg.hTask); disp(double(wg.hTask.isTaskDone())); else; disp(1); end",
            "    if hPs.active;",
            "        pause(0.2);",
            "        disp('Photostim sequence position after single pulse:');",
            "        if isempty(hPs.sequencePosition); disp('NaN'); else; disp(double(hPs.sequencePosition)); end",
            "        disp('Photostim completed sequences after single pulse:');",
            "        if isempty(hPs.completedSequences); disp('NaN'); else; disp(double(hPs.completedSequences)); end",
            "        waveformAdvanced = false;",
            "        if ~isempty(waveformSequencePositionBefore) && ~isempty(hPs.sequencePosition);",
            "            waveformAdvanced = double(hPs.sequencePosition) > double(waveformSequencePositionBefore);",
            "        end",
            "        if ~waveformAdvanced && ~isempty(waveformCompletedSequencesBefore) && ~isempty(hPs.completedSequences);",
            "            waveformAdvanced = double(hPs.completedSequences) > double(waveformCompletedSequencesBefore);",
            "        end",
            "        disp('Single pulse advanced photostim:');",
            "        disp(double(waveformAdvanced));",
            "    else;",
            "        disp('Photostim single-pulse probe skipped: photostim not active');",
            "        disp('Single pulse advanced photostim:');",
            "        disp(0);",
            "    end",
            "end",
            "trialTriggerTimesSec = [0.1 1.1 2.1 3.1 4.1];",
            f"trialPulseWidthSec = {path_config.trial_waveform_pulse_width_ms / 1000.0!r};",
            "trialTotalDurationSec = 5.2;",
            "assignin('base', 'photostimTrialTriggerTimesSec', trialTriggerTimesSec);",
            "assignin('base', 'photostimTrialPulseWidthSec', trialPulseWidthSec);",
            "assignin('base', 'photostimTrialTotalDurationSec', trialTotalDurationSec);",
            "if most.idioms.isValidObj(wg.hTask) && wg.hTask.active; wg.hTask.stop(); end",
            ...
            "disp(trialTotalDurationSec);",
            "disp('Waveform train buffer length samples:');",
            "disp(numel(wv));",
            "disp('Waveform train buffer high samples:');",
            "disp(sum(wv > 0));",
            "wg.startTask();",
            "disp('Waveform train test task started');",
            "t0 = tic;",
            "while most.idioms.isValidObj(wg.hTask) && ~wg.hTask.isTaskDone() && toc(t0) < (trialTotalDurationSec + 1.0); pause(0.01); end",
            "disp('Waveform task active after train:');",
            "if most.idioms.isValidObj(wg.hTask); disp(double(wg.hTask.active)); else; disp(0); end",
            "disp('Waveform task done after train:');",
            "if most.idioms.isValidObj(wg.hTask); disp(double(wg.hTask.isTaskDone())); else; disp(1); end",
            "if hPs.active;",
            "    pause(0.2);",
            ...
            "    deliveredCount = 0;",
            "    if ~isempty(waveformSequencePositionBefore) && ~isempty(hPs.sequencePosition);",
            "        deliveredCount = max(0, double(hPs.sequencePosition) - double(waveformSequencePositionBefore));",
            "    end",
            "    disp('Waveform train advanced photostim count:');",
            "    disp(deliveredCount);",
            "else;",
            "    disp('Photostim train probe skipped: photostim not active');",
            "    disp('Waveform train advanced photostim count:');",
            "    disp(0);",
            "end",
